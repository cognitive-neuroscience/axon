...
import {
    DemographicsQuestionnaireResponse,
...
export class QuestionnaireService {
    ...
    private _questionnairesSubject: BehaviorSubject<Questionnaire[]>;
    public questionnaires: Observable<Questionnaire[]>;

    constructor(private http: HttpClient) {
        this._questionnairesSubject = new BehaviorSubject(null);
        this.questionnaires = this._questionnairesSubject.asObservable();
    }
    ...
    update(): void {
        this._getQuestionnaires().subscribe((questionnaires) => {
            ...
            this._questionnairesSubject.next(questionnaires);
        });
    }
    ...
    saveDemographicsQuestionnaireResponse(response: DemographicsQuestionnaireResponse): Observable<boolean> {
        return this.http
            .post(`${environment.apiBaseURL}${this.RESOURCE_PATH}demographics`, response, { observe: "response" })
            .pipe(map((x) => x.ok));
    }

    saveFeedQuestionnaireResponse(response: FeedbackQuestionnaireResponse): Observable<boolean> {
        return this.http
            .post(`${environment.apiBaseURL}${this.RESOURCE_PATH}feedback`, response, { observe: "response" })
            .pipe(map((x) => x.ok));
    }
