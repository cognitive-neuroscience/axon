...
import { postChoiceRatingTask } from '../../../models/TaskData';
...
@Component({
  selector: 'app-everyday-choice3',
  templateUrl: './everyday-choice3.component.html',
  styleUrls: ['./everyday-choice3.component.scss']
})
export class EverydayChoice3Component implements OnInit {
  ...
  generateStimulus() {
    ...
    this.currentActivity = this.currentSet[this.currentTrial - 1];
      ...
      if (this.done_one_part == false) {
        this.ratingType = 'effort'
      }
      else {
        this.ratingType = 'frequency'
      }
  ...
  uploadResults(data: postChoiceRatingTask[]): Observable<boolean> {
    const experimentCode = this.taskManager.getExperimentCode()
    return this.uploadDataService.uploadData(experimentCode, "postChoiceRatingTask", data).pipe(
      map(ok => ok.ok)
    )
  }
