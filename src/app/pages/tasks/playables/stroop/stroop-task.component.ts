...
export class StroopTaskComponent implements OnInit {
    ...
    onKeyPress(event: KeyboardEvent) {
        if (this.isValidKey(event.key) && this.isResponseAllowed) {
            ...
            switch (event.key) {
                case Key.NUMONE:
                    lastElement.userAnswer = UserResponse.RED;
                    break;
                case Key.NUMTWO:
                    lastElement.userAnswer = UserResponse.BLUE;
                    break;
                case Key.NUMTHREE:
                    lastElement.userAnswer = UserResponse.GREEN;
                    break;
                default:
                    lastElement.userAnswer = UserResponse.INVALID;
                    break;
            }
    ...
    generateStimulus() {
        ...
        this.data.push({
            ...
            actualAnswer:
                this.color.toUpperCase() === UserResponse.RED
                    ? UserResponse.RED
                    : this.color.toUpperCase() === UserResponse.BLUE
                    ? UserResponse.BLUE
                    : UserResponse.GREEN,
    ...
    uploadResults(data: StroopTask[]): Observable<boolean> {
        ...
        return this.uploadDataService.uploadData(studyCode, TaskNames.STROOP, data).pipe(map((ok) => ok.ok));
    }
