...
import { Feedback, JitteredInterval, Key, UserResponse } from "src/app/models/InternalDTOs";
...
export class OddballComponent implements OnInit {
    ...
    durationFixationPresented: JitteredInterval = environment.production
        ? { lowerbound: 1000, upperbound: 2000 }
        : { lowerbound: 100, upperbound: 500 };
    ...
    ngOnInit() {
        ...
        BlockGenerator.initialize(this.http).subscribe(
            (data) => {
    ...
    async startActualGame() {
        ...
        this.trials = this.includeNovelStimuli
            ? new BlockGenerator(this.targetImage, 6, 6, 60, this.novelStimuliUsed).trials
            : new BlockGenerator(this.targetImage, 12, 0, 60).trials;
        this.block++;
    ...
    async showStimulus() {
        ...
        await this.wait(
            getRandomNumber(this.durationFixationPresented.lowerbound, this.durationFixationPresented.upperbound)
        );
        ...
        this.stimulusShownTimeout = window.setTimeout(() => {
            clearTimeout(this.stimulusShownTimeout);
    ...
    uploadResults(data: Oddball[]): Observable<boolean> {
        ...
        return this.uploadDataService.uploadData(studyCode, TaskNames.ODDBALL, data).pipe(map((ok) => ok.ok));
    }
