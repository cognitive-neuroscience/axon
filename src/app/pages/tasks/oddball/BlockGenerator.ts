...
export class BlockGenerator {
    ...
    public static initialize(http: HttpClient): Observable<boolean> {
        ...
        const getArray = BlockGenerator.imageNames.map(img => BlockGenerator.http.get(`/assets/images/stimuli/oddball/${img}`, { responseType: 'blob' }))
        ...
        return forkJoin(getArray).pipe(
            ...
            map(arr => {
                arr.forEach((element: Blob, index) => {
                    ...
                    const imageBlobs = (name === 'square.png' || name === 'triangle.png' ? BlockGenerator.stimuliBlobs : BlockGenerator.novelBlobs);
    ...
    constructor(targetTrial: 'square.png' | 'triangle.png', numTargetTrials: number, numNovelTrials: number, numTotalTrials: number, scenesToExclude: string[] = []) {
        ...
        if(numNovelTrials > (this.novelBlobSize - scenesToExclude.length)) throw new Error("Cannot create given number of novel stimuli without repeats")
        ...
        this.trials.fill({
            ...
            blob: targetTrial === 'square.png' ? BlockGenerator.stimuliBlobs['triangle.png'] : BlockGenerator.stimuliBlobs['square.png'],
            isTarget: false
        })
