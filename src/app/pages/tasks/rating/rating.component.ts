...
export class RatingComponent implements OnInit {
  ...
  generateStimulus() {
    ...
    this.data.push({
      ...
      ratingType: this.counterbalance == 0 ? (this.partOneComplete ? 'should' : 'like') : (this.partOneComplete ? 'like' : 'should') ,
  ...
  uploadResults(data: RatingTask[]): Observable<boolean> {
    ...
    return this.uploadDataService.uploadData(experimentCode, TaskNames.RATING, data).pipe(
      map(ok => ok.ok)
    )
  }
