...
export class DemographicsQuestionnaireComponent implements OnInit {
    ...
    demographicsQuestionnaire = this.fb.group({
        age: [
            "",
            [Validators.required, Validators.min(18), Validators.max(122), Validators.pattern(this.numbersRegex)],
        ],
        sex: ["", Validators.required],
        selfIdentification: [[], Validators.required],
        yearsOfEducation: [
            "",
            [Validators.required, Validators.min(0), Validators.max(30), Validators.pattern(this.numbersRegex)],
        ],
        hasNeuroConditions: ["", [Validators.required]],
        hasPsychConditions: ["", [Validators.required]],
    ...
    submit() {
        ...
        const response: DemographicsQuestionnaireResponse = {
            ...
            currentEmploymentStatus: this.demographicsQuestionnaire.get("currentEmploymentStatus").value,
            householdIncome: this.demographicsQuestionnaire.get("householdIncome").value,
            socialClass: this.demographicsQuestionnaire.get("socialClass").value,
        };
        this.questionnaireService
            .saveDemographicsQuestionnaireResponse(response)
            .pipe(take(1))
            .subscribe(
                (ok) => {
                    if (ok) {
