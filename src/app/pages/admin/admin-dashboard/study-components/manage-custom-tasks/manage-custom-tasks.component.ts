...
export class ManageCustomTasksComponent implements OnInit {
    ...
    openCreateCustomTaskModal() {
        this.dialog
            .open(CreateCustomTaskDialogComponent, { width: "30%" })
            .afterClosed()
            .subscribe((customTask: CustomTask) => {
                if (customTask) this._createCustomTask(customTask);
            });
    }

    private _createCustomTask(customTask: CustomTask) {
        this.customTaskService.createQuestionnaire(customTask).subscribe(
            (data) => {
                this.customTaskService.update();
    ...
    deleteCustomTask(customTask: CustomTask) {
        this.confirmationService
            .openConfirmationDialog("Are you sure you want to delete the task: " + customTask.name + "?")
            .pipe(
                mergeMap((ok) => {
                    if (ok) {
